...
public class ExampleFlightProducer implements FlightProducer {
  ...
  public void getStream(CallContext callContext, Ticket ticket, ServerStreamListener serverStreamListener) {
    ...
    if (Arrays.equals(str.getBytes(), ticket.getBytes())) {
      try (final VectorSchemaRoot vectorSchemaRoot = createVectors()) {
        serverStreamListener.start(vectorSchemaRoot);
        serverStreamListener.putNext();
        serverStreamListener.completed();
      }
    } else {
      serverStreamListener.error(CallStatus.NOT_FOUND.toRuntimeException());
    }
  }
  ...
  private VectorSchemaRoot createVectors() {
    ...
    VectorSchemaRoot vectorSchemaRoot = new VectorSchemaRoot(asList(vecStr, vecInt));
